 ...
 * peripheral specific operations etc
 ¡Á
 * \brief     Generic Upp Driver for SYS/BIOS Platform.It uses the internal DMA
 *            for data transfer.
 *
 *            This file implements the UPP IOM driver to be used with
 *            SYS-BIOS 6 operating system.
 *
 *            (C) Copyright 2013, Shanghai TestEasy, Inc
 *
 * \author    zzw
 *
 * \version   0.1
...
const Upp_Params Upp_PARAMS =
{
    ...
    CSL_SYSCLK_2_FREQ,
    9,
    Upp_intHandlerSel_ECM,	//Upp_intHandlerSel_HWI,	//
    Upp_pllDomain_NONE
};
...
static Int uppMdBindDev(Ptr *devp, Int devid, Ptr devParams)
{
    ...
    if (IOM_COMPLETED == status)
    {
        ...
        Queue_construct(&(instHandle->chanA.queActiveList), NULL);	//zzw
        Queue_construct(&(instHandle->chanA.quePendList), NULL);	//zzw

        Queue_construct(&(instHandle->chanB.queActiveList), NULL);	//zzw
        Queue_construct(&(instHandle->chanB.quePendList), NULL);	//zzw
...
//    HWI_Attrs            hwiAttrs   = HWI_ATTRS;
    ...
    /* Enabling the HWI_ID                                                    */
    Hwi_enableIER(1 << instHandle->devParams.hwiNumber);
    //----------------------------------------------------------zzw
...
static Void uppUnRegisterInt(Uint16 evt)
{
    ...
    EventCombiner_disableEvent(evt);	//zzw
    ...
    while (status)
    {
            ...
            (TRUE != chanHandle->flushAbort))
        {
            if ((CSL_UPP_UPIS2_PEND_MASK !=
                ...
                //(FALSE == instHandle->pwrmInfo.ioSuspend))	//zzw
            {
                /* check if there is any packet to load or not                */
                //if (FALSE == QUE_empty(&(chanHandle->quePendList)))
                if (FALSE == Queue_empty(Queue_handle(&chanHandle->quePendList)))	//zzw
                {
                    //ioPacket = QUE_get(&(chanHandle->quePendList));
            		ioPacket = (IOM_Packet *)Queue_get(Queue_handle(&(chanHandle->quePendList)));//zzw

                    /* put the packet in to the active list                   */
                    //QUE_put(&(chanHandle->queActiveList),ioPacket);
                    Queue_put(Queue_handle(&(chanHandle->queActiveList)),(Ptr)ioPacket); //zzw
                                 ...
                                 Uint32       status)
{
    ...
    if (FALSE == Queue_empty(Queue_handle(&chanHandle->queActiveList)))	//zzw
    {
        //ioPacket = QUE_get(&(chanHandle->queActiveList));
		ioPacket = (IOM_Packet *)Queue_get(Queue_handle(&(chanHandle->queActiveList)));//zzw
                          ...
                          IOM_Packet     *ioPacket)
{
        ...
        (Upp_DriverState_PWRM_SUSPEND == instHandle->devState))
    {
        ...
        Queue_put(Queue_handle(&(chanHandle->quePendList)),(Ptr)ioPacket); //zzw
