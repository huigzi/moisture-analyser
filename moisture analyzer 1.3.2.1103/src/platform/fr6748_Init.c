...
#define PINMUX19_VALUE 	0x00081100

#define PINMUX3_SPI_ENABLE       0x00001101u
#define PINMUX4_CS_ENABLE        0x00000040u
#define PINMUX0_AD_RDY           0x00000008u
#define PINMUX5_PWM              0x00000020u
#define PINMUX13_FAN             0x00000808u
...
void configureGuaoC6748(void)
{
  ...
  //  CSL_GpioRegsOvly     gpioRegs = (CSL_GpioRegsOvly)(CSL_GPIO_0_REGS);
    ...
    syscfgRegs->PINMUX3 = (PINMUX3_VALUE | PINMUX3_SPI_ENABLE);
    syscfgRegs->PINMUX4 = (PINMUX4_VALUE | PINMUX4_CS_ENABLE);
    syscfgRegs->PINMUX5 = (PINMUX5_VALUE | PINMUX5_PWM);
...
//    Psc_ModuleClkCtrl((Psc_DevId)CSL_LPSC_INST_EMIFA_0, CSL_PSC_EMIFA, TRUE);
    ...
    CSL_FINS(gpioRegs->BANK[3].DIR,GPIO_DIR_DIR2,0); // FAN(out)
    ...
    CSL_FINS(gpioRegs->BANK[3].DIR,GPIO_DIR_DIR15,0);   //8710A(out)
    CSL_FINS(gpioRegs->BANK[3].DIR,GPIO_DIR_DIR13,0);  //CMOS_PWREN(out)


    CSL_FINS(gpioRegs->BANK[0].OUT_DATA,GPIO_OUT_DATA_OUT11,1);	//AD_RST(out)
    CSL_FINS(gpioRegs->BANK[0].OUT_DATA,GPIO_OUT_DATA_OUT12,0);	//DIR_IN(out)
    CSL_FINS(gpioRegs->BANK[1].OUT_DATA,GPIO_OUT_DATA_OUT15,1);	//RE#
    CSL_FINS(gpioRegs->BANK[3].OUT_DATA,GPIO_OUT_DATA_OUT2,1); // FAN(out)
    ...
    CSL_FINS(gpioRegs->BANK[3].OUT_DATA,GPIO_OUT_DATA_OUT15,1); //8710A(out)
    CSL_FINS(gpioRegs->BANK[3].OUT_DATA,GPIO_OUT_DATA_OUT13,1); // CMOS_PWREN(out)
    ...
    pwmRegs->TBCTL = CSL_FMK(EHRPWM_TBCTL_CTRMODE, CSL_EHRPWM_TBCTL_CTRMODE_UP)
        						|CSL_FMK(EHRPWM_TBCTL_PHSEN, CSL_EHRPWM_TBCTL_PHSEN_DISABLE)
        						|CSL_FMK(EHRPWM_TBCTL_PRDLD, CSL_EHRPWM_TBCTL_PRDLD_FROMSHADOW)
        						|CSL_FMK(EHRPWM_TBCTL_SYNCOSEL, CSL_EHRPWM_TBCTL_SYNCOSEL_DISABLE)
        						|CSL_FMK(EHRPWM_TBCTL_HSPCLKDIV, CSL_EHRPWM_TBCTL_HSPCLKDIV_DIVBY1)
        						|CSL_FMK(EHRPWM_TBCTL_CLKDIV, CSL_EHRPWM_TBCTL_CLKDIV_DIVBY16)
                                |CSL_FMK(EHRPWM_TBCTL_FREE_SOFT, CSL_EHRPWM_TBCTL_FREE_SOFT_RUNFREE);

    CSL_FINS(pwmRegs->TBPRD,EHRPWM_TBPRD_TBPRD,20000);
    CSL_FINS(pwmRegs->TBPHS,EHRPWM_TBPHS_TBPHS,0);
    CSL_FINS(pwmRegs->TBCTR,EHRPWM_TBCTR_TBCTR,0);

    pwmRegs->CMPCTL = CSL_FMK(EHRPWM_CMPCTL_SHDWAMODE,CSL_EHRPWM_CMPCTL_SHDWAMODE_IMMEDIATE )
        						|CSL_FMK(EHRPWM_CMPCTL_LOADAMODE, CSL_EHRPWM_CMPCTL_LOADAMODE_FREEZE);
    pwmRegs->AQCTLB = CSL_FMK(EHRPWM_AQCTLB_ZRO, CSL_EHRPWM_AQCTLB_ZRO_EPWMXBHIGH)
                                |CSL_FMK(EHRPWM_AQCTLB_CBU,CSL_EHRPWM_AQCTLB_CBU_EPWMXBLOW );

    pwmRegs->DBCTL = CSL_FMK(EHRPWM_DBCTL_OUT_MODE,CSL_EHRPWM_DBCTL_OUT_MODE_BYPASS);
    pwmRegs->PCCTL = CSL_FMK(EHRPWM_PCCTL_CHPEN,CSL_EHRPWM_PCCTL_CHPEN_DISABLE);
