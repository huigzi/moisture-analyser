...
//    Psc_ModuleClkCtrl((Psc_DevId)CSL_LPSC_INST_EMIFA_0, CSL_PSC_EMIFA, TRUE);
    ...
    CSL_FINS(gpioRegs->BANK[0].DIR,GPIO_DIR_DIR0,0);	//FAN_CONTROL(out)		GP0(0)
    ...
    CSL_FINS(gpioRegs->BANK[3].DIR,GPIO_DIR_DIR15,0);   //8710A(in)


    CSL_FINS(gpioRegs->BANK[0].OUT_DATA,GPIO_OUT_DATA_OUT0,1);      //FAN(out)
